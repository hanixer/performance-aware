...
fn getOSTimerWindows() u64 {
    var timer: i64 = undefined;
    _ = std.os.windows.ntdll.RtlQueryPerformanceCounter(&timer);
    return @as(u64, @intCast(timer));
}

fn getCPUFrequencyWindows(msToWait: u64) u64 {
    const cpuStart = getOSTimerWindows();
    var freq: i64 = 0;
    ...
    const cpuEnd = getOSTimerWindows();
...
const ProfilerData = struct {
    ...
    fn startProfile(self: *ProfilerData) void {
        self.profile_start = getOSTimerWindows();
    }

    fn endProfile(self: *ProfilerData) void {
        self.profile_end = getOSTimerWindows();
    }
    ...
    fn endZone(self: *ProfilerData, zone: *ProfilerZone) void {
        ...
        const end_time = getOSTimerWindows();
