...
pub fn main() !void {
    ...
    if (std.fs.cwd().openFile(read_params.file_name, .{})) |file| {
        ...
        if (windows.VirtualAlloc(null, stat.size, windows.MEM_COMMIT | windows.MEM_RESERVE, windows.PAGE_READWRITE)) |ptr| {
            const ptr2: [*]u8 = @ptrCast(ptr);
            read_params.dest = ptr2[0..stat.size];
            ptr2[0] = 3;
        } else |_| {
            std.debug.print("*** {d}", .{windows.kernel32.GetLastError()});
            return;
        }
    } else |_| {
