...
const RepetitionValueType = enum {
    test_count,
    cpu_timer,
    mem_page_faults,
    byte_count,
...
const RepetitionValue = struct {
    e: [RepetitionValueType.count]u64,
};
...
pub fn main() !void {
    ...
    var testers = [_]RepetitionTester{RepetitionTester{}} ** 4;
    ...
    while (true) {
        ...
        try stdout.print("\n--- read via readAll + alloc ---\n", .{});
        testers[2].newTestWave(read_params.dest.len, cpu_timer_freq);
        try readViaReadAllAllocWithAlloc(&testers[2], read_params);
        
        try stdout.print("\n--- read via readAll + alloc zig ---\n", .{});
        testers[3].newTestWave(read_params.dest.len, cpu_timer_freq);
        try readViaReadAllAllocWithAllocZig(&testers[3], read_params, gpa.backing_allocator);
    }
}