...
extern fn movAllBytesAsm(ptr: [*]u8, count: usize) void;
extern fn cmpAllBytesAsm(ptr: [*]u8, count: usize) void;
...
const RepetitionTester = struct {
    ...
    seconds_to_wait_for_update: u64 = 10,
...
fn windowsVirtualAlloc(size: u64) ![]u8 {
    const ptr = try windows.VirtualAlloc(null, size, windows.MEM_COMMIT | windows.MEM_RESERVE, windows.PAGE_READWRITE);
    const ptr2: [*]u8 = @ptrCast(ptr);
    const result = ptr2[0..size];
    return result;
}

fn windowsVirtualFree(data: []u8) void {
    windows.VirtualFree(data.ptr, 0, windows.MEM_RELEASE);
}
...
fn readViaReadAllAllocWithAlloc(tester: *RepetitionTester, read_params: ReadParameters) !void {
    while (try tester.isTesting()) {
        if (std.fs.cwd().openFile(read_params.file_name, .{})) |file| {
            const dest = try windowsVirtualAlloc(read_params.dest.len);
            tester.beginTime();
            if (file.readAll(dest)) |count| {
                tester.endTime();
                tester.countBytes(count);
            } else |_| {
                tester.endTime();
...
fn readViaReadAllAllocWithAllocZig(tester: *RepetitionTester, read_params: ReadParameters, allocator: std.mem.Allocator) !void {
    while (try tester.isTesting()) {
        if (std.fs.cwd().openFile(read_params.file_name, .{})) |file| {
            const dest = try allocator.alloc(u8, read_params.dest.len);
            tester.beginTime();
            if (file.readAll(dest)) |count| {
                tester.endTime();
                tester.countBytes(count);
...
fn writeAllBytes(tester: *RepetitionTester, read_params: ReadParameters) !void {
    while (try tester.isTesting()) {
        const dest = read_params.dest;
        const count = dest.len;
        tester.beginTime();
...
fn writeAllBytesAsm(tester: *RepetitionTester, read_params: ReadParameters) !void {
    while (try tester.isTesting()) {
        const dest = read_params.dest;
        const count = dest.len;
        tester.beginTime();

        movAllBytesAsm(dest.ptr, dest.len);

        tester.endTime();
        tester.countBytes(count);
    }
}
fn cmpAllBytes(tester: *RepetitionTester, read_params: ReadParameters) !void {
    while (try tester.isTesting()) {
        const dest = read_params.dest;
        const count = dest.len;
        tester.beginTime();

        cmpAllBytesAsm(dest.ptr, dest.len);
...
fn writeAllBytesAlloc(tester: *RepetitionTester, read_params: ReadParameters) !void {
    while (try tester.isTesting()) {
        const dest = try windowsVirtualAlloc(read_params.dest.len);
        tester.beginTime();
...
pub fn main() !void {
    ...
    if (std.fs.cwd().openFile(read_params.file_name, .{})) |file| {
        ...
        if (windowsVirtualAlloc(stat.size)) |allocced| {
            read_params.dest = allocced;
        } else |_| {
            return;
        }
    } else |_| {
    ...
    var testers = [_]RepetitionTester{RepetitionTester{}} ** 5;
    ...
    while (true) {
        ...
        try writeAllBytes(&testers[1], read_params);
        
        try stdout.print("\n--- write all asm ---\n", .{});
        testers[2].newTestWave(read_params.dest.len, cpu_timer_freq);
        try writeAllBytesAsm(&testers[2], read_params);

        try stdout.print("\n--- cmp all asm ---\n", .{});
        testers[3].newTestWave(read_params.dest.len, cpu_timer_freq);
        try cmpAllBytes(&testers[3], read_params);
    }
}